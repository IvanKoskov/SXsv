...
use ratatui::{
    ...
    text::{Line, Text},
    widgets::{
        Block, BorderType, Borders, List, ListDirection, ListState, Padding, Paragraph, Wrap, block,
    },
};
...
pub fn run_info(terminal: &mut DefaultTerminal) -> Result<()> {
    let read_time_when_installed = || -> String {
        ...
        let contents = read_to_string(path_to_time).unwrap_or_else(|_| "Unknown".to_string());
        contents
    };
    ...
    loop {
        terminal.draw(|frame: &mut Frame| {
            let b = Block::default()
                .title(
                    Line::from("Build 05312025 (1.0.0)")
                        .left_aligned()
                        .style(Style::new().red()),
                )
                .title(
                    Line::from("SXSV - fast and neat data formats viewer and editor")
                        .centered()
                        .bold(),
                )
                .borders(Borders::ALL)
            ...
            let body = vec![
                Line::from(format!("Platform: {}", std::env::consts::OS)),
                Line::from(format!(
                    "Installed current version on: {:?}",
                    read_time_when_installed()
                )),
                Line::from("Main developer and  maintainer: Ivan Koskov"),
                Line::from("License:  BSD-3-Clause license"),
                Line::from("Copyright (c) [2025], [Ivan Koskov aka Evan Matthew]"),
            ];
            let par = Paragraph::new(body)
                .block(b.clone().padding(Padding::new(
                    0, // left
                    0, // right
                    2, // top
                    0, // bottom
